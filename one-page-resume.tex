...
\usepackage[top=4.5mm,bottom=4.5mm,left=5.5mm,right=5.5mm]{geometry}
\usepackage{fontspec}
\usepackage{xeCJK}
\usepackage{xcolor}
\usepackage{graphicx}
...
\newcommand{\sect}[3]{%
  \noindent{\large\bfseries\textcolor{#2}{#1}}\hspace{1mm}{\large\bfseries #3}\par\vspace{0.45mm}
}
\newcommand{\chip}[2]{%
  \tcbox[
    enhanced,
...
\newcommand{\tinyline}[2]{%
  {\scriptsize\textbf{#1}}\ {\scriptsize #2}\par
}
\newcommand{\proofimg}[3]{%
  \begin{minipage}[t]{#1}
    \centering
    \fcolorbox{line}{white}{\includegraphics[width=\dimexpr\linewidth-2\fboxsep-2\fboxrule,height=20mm,keepaspectratio]{#2}}\\[-.3mm]
...
\begin{minipage}[t]{36mm}
  \begin{colorboxx}[height=43mm,valign=center,halign=center]{blueA}
    \includegraphics[width=31mm,height=39mm,keepaspectratio]{assets/profile-photo.png}
  \end{colorboxx}
\end{minipage}\hfill
...
\begin{minipage}[t]{45mm}
  \begin{colorboxx}[height=43mm,valign=center]{purpleA}
    \includegraphics[width=\linewidth,height=22mm,keepaspectratio]{assets/esgpb-chatbot-screenshot.png}\par\vspace{.6mm}
    {\scriptsize\bfseries\textcolor{purpleA}{可展示系統}}\par
    {\scriptsize ESGPB 商家推薦助理、智慧旅遊、會議秘書、醫療工作坊與鐵道 AI 技術服務。}
  \end{colorboxx}
\end{minipage}
...
\begin{colorboxx}{purpleA}
  ...
  \begin{colorboxx}[height=34mm]{blueA}
    {\small\bfseries 全域意境感知智慧會議秘書系統}\par
    {\scriptsize AIGO 佳作相關技術文件}\par\vspace{.8mm}
    {\scriptsize
    技術：Whisper、說話者分離、Sentence-BERT、TextRank、LLM、n8n / Webhook。\par
    內容：會前資料包、會中摘要、會後任務追蹤。}
  \end{colorboxx}
  &
  \begin{colorboxx}[height=34mm]{cyanA}
    {\small\bfseries ESGPB 虛擬人商家推薦助理}\par
    {\scriptsize 好奇兄弟雲端合作}\par\vspace{.8mm}
    {\scriptsize
    技術：RAG、Qdrant、Redis、PostgreSQL、GPT Realtime、Sora。\par
    內容：商家資料 API、優惠推薦、語音 / 文字 / 拍照互動。}
  \end{colorboxx}
  &
  \begin{colorboxx}[height=34mm]{purpleA}
    {\small\bfseries InnoServe 第三名｜多模態旅遊 AI Agent}\par
    {\scriptsize 具意圖探測的多模態旅遊問答服務系統}\par\vspace{.8mm}
    {\scriptsize
    技術：Neo4j、Qdrant、Redis、PostgreSQL、Whisper、Blip2、LLM。\par
    內容：行程問答、導遊減勞、旅後分析。}
  \end{colorboxx}
  &
  \begin{colorboxx}[height=34mm]{orangeA}
    {\small\bfseries 112 霸凌偵測及專注力辨識系統}\par
    {\scriptsize 精英國際教育集團｜科技部案}\par\vspace{.8mm}
    {\scriptsize
    技術：STT、BERT、MFCC、CNN/LSTM、3DCNN、多模態融合。\par
    內容：疑似霸凌與專注力狀態偵測。}
  \end{colorboxx}
  \end{tabularx}
\end{colorboxx}
